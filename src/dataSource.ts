...
export class WeatherAPI extends RESTDataSource {
  ...
  async withCoords(lat: number, lon: number) {
    const data = await this.get('weather', { lat, lon, units:"metric" })
  ...
  async dailyWeather(lat: number, lon: number) {
    const data = await this.get('onecall', { lat, lon, units:"metric", exclude:"current,minutely,alerts" })
    return camelCaseKeys(data, { deep: true })
  }
